...
/*  Created On : 10-oct.-2021 12:15:30 p. m. 				*/
/*  DBMS       : Oracle 						*/
/* ---------------------------------------------------- */
...
SELECT COUNT(*) INTO C 
FROM USER_TRIGGERS 
  WHERE TRIGGER_NAME = 'TRG_Calificacion_ID_CALIFICACION'; 
  IF (C > 0) THEN 
    EXECUTE IMMEDIATE 'DROP TRIGGER "TRG_Calificacion_ID_CALIFICACION"'; 
END IF; 
END;
/
...
SELECT COUNT(*) INTO C 
FROM USER_SEQUENCES 
  WHERE SEQUENCE_NAME = 'SEQ_Calificacion_ID_CALIFICACION'; 
  IF (C > 0) THEN 
    EXECUTE IMMEDIATE 'DROP SEQUENCE "SEQ_Calificacion_ID_CALIFICACION"'; 
END IF; 
END;
/
...
(
	"ID_CALIFICACION" NUMBER(8) NOT NULL,
...
(
	...
	"ID_TRANSFERENCIA" NUMBER(10) NULL,
	"ID_PSE" NUMBER(10) NULL,
	"CORREO_PSE" VARCHAR(30) NULL
)
;
...
(
	...
	"MONTO" NUMBER(11) DEFAULT 0 NOT NULL,	-- Precio total del pedido (Calculado por la Base de Datos)
...
(
	...
	"PROM_CALIFICACION" NUMBER(2,1) DEFAULT 0 NOT NULL,
	"NUM_PEDIDOS" NUMBER(5) DEFAULT 0 NOT NULL,
	"COMISION" NUMBER(11) DEFAULT 0 NOT NULL
)
;

/* Create Comments, Sequences and Triggers for Autonumber Columns */

CREATE SEQUENCE "SEQ_Calificacion_ID_CALIFICACION" 
	INCREMENT BY 1 
	START WITH 1 
	NOMAXVALUE 
	MINVALUE  1 
...
CREATE OR REPLACE TRIGGER "TRG_Calificacion_ID_CALIFICACION" 
	BEFORE INSERT 
	ON "Calificacion" 
	FOR EACH ROW 
	BEGIN 
		SELECT "SEQ_Calificacion_ID_CALIFICACION".NEXTVAL 
...
CREATE INDEX "IXFK_Pago_Pedido"   
 ON  "Pago" ("FK_ID_PEDIDO") 
;
...
ALTER TABLE  "RepresentantePeriodo" 
 ADD CONSTRAINT "CK_REPRESENTANTEPERIODO" CHECK (VALOR_RECAUDADO > 0)
;

ALTER TABLE  "RepresentantePeriodo" 
 ADD CONSTRAINT "CK_GRADO" CHECK (GRADO IN ('beginner','junior','senior','master'))
;
